...
//				"CREATE TABLE IF NOT EXISTS ProgramListTag (id integer primary key autoincrement, sid INTEGER UNIQUE, tag varchar(255), dspwd char(1))");
	...
	public Map<String, String> GetProgramListByTags(String Tag) {
		...
		try {
			db = getReadableDatabase();
			Cursor Row;
			if(Configs.canSearchLastMenu){
				Row = db.rawQuery("Select sid,dspwd from ProgramListTag where tag like ?",
						new String[] { "%" + Tag + "%" });
			}else{
				Row = db.rawQuery("Select sid,dspwd from ProgramListTag where dspwd!='1' and tag like ?",
						new String[] { "%" + Tag + "%" });
			}
